...
int EntranceRange(int[] arr, int min, int max)
{
    int count = 0;
...
Console.WriteLine($"В массиве чисел: \n{string.Join(", ", array)}");
int min = 10;
int max = 99;
int counter = EntranceRange(array, min, max);
Console.WriteLine($"{counter} из них в отрезке [{min}, {max}]");
